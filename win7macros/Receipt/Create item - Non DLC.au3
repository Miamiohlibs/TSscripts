...
TraySetIcon(@DesktopDir & "\Receipt\Images\red N.ico")
...
Dim $decide
Dim $BARCODE
Dim $approval, $notification, $firm
Dim $T25
...
If $300_E = "none" or $300_E_E = "" Then
		sleep(0100)
Else
	$IS_MUS = StringInStr($BIB_LOC_1, "mul")
	If $IS_MUS = 0 Then
...
Select
	...
	Case $approval = 1 Or $notification = 1
		MsgBox(0, "Copy Cataloging", "Place pink copy in book." & @CR & $T85 & @CR & $T86 & @CR & $T87 & @CR & $T88 & @CR & $T89)
	Case $reserve = 1
