...
Switch $decide
	Case "O" ;OCLC #
		...
		Else
			MsgBox(64, "No 001 field", "The record does not have a 001 field. Please close the message box, run the script again, and use another search option.")
			Exit
		EndIf
...
;~ 		$search_2 = "http://uclid.uc.edu/search/o?SEARCH=" & $OCLC_NUM & "&submit.x=48&submit.y=16&searchscope=36"
	...
	Case "T" ;Title search (245 |a)
		...
		If $245_A > -1 Then
			...
			EndIf
		Else
			MsgBox(64, "No 245 field", "The record does not have a 245 field. Please close the message box, run the script again, and use another search option.")
			Exit
		EndIf
;~ 			$search_1 = "http://wsuol2.wright.edu/search/t?SEARCH=" & $245 & "&sortdropdown=-&searchscope=7&submit=Search"
			$search_1 = "http://uclid.uc.edu/search/t?SEARCH=" & $245 & "&submit.x=48&submit.y=16&searchscope=36"
...
;~ 			$search_1 = "http://wsuol2.wright.edu/search/i?SEARCH=" & $ISBN & "&sortdropdown=-&searchscope=7&submit=Search"
			$search_1 = "http://uclid.uc.edu/search/i?SEARCH=" & $ISBN & "&submit.x=48&submit.y=16&searchscope=36"
			$search_2 = "http://www.lib.muohio.edu/multifacet/books/"& $ISBN & "?field=isn&sort=relevance&fn[]=item_campus&fv[]=SWORD"
		Else
			MsgBox(64, "No ISBN/ISSN", "There was no ISBN/ISSN entered. Please close the message box, run the script again, and use another search option.")
			Exit
		EndIf
	Case Else
		Exit
