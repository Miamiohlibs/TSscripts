...
GUICtrlSetImage(-1, @WorkingDir & "\scripts\Images\redN.ico")
GUICtrlSetOnEvent($5, "nonDLC")

;$6 = GUICtrlCreateButton("Sample button", 270, 10, 50, 50, $BS_ICON)
;GUICtrlSetImage(-1, @WorkingDir & "\scripts\Images\rda.ico")
;GUICtrlSetOnEvent($6, "rda")

$7 = GUICtrlCreateButton("Sample button", 270, 10, 50, 50, $BS_ICON)
...
Func isbntotitle()
	Sleep(0400)
	Run(@WorkingDir & "\scripts\isbnToTitle.exe")
EndFunc
...
Func DLC()
	Sleep(0400)
	Run(@WorkingDir & "\scripts\Create Item - DLC.exe")
EndFunc

Func nonDLC()
	Sleep(0400)
	Run(@WorkingDir & "\scripts\Create item - Non DLC.exe")
EndFunc

Func rda()
	Sleep(0400)
	Run(@WorkingDir & "\scripts\rda.exe")
EndFunc
